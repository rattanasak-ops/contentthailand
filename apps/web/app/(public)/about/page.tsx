  ...
  return (
    <div className="min-h-screen bg-midnight pt-8 pb-20">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Intro */}
        <div className="bg-navy/40 rounded-2xl border border-white/5 p-8 mb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
          <div className="bg-navy/40 rounded-xl border border-white/5 p-6">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-thai font-bold text-white">{lang === "th" ? "วิสัยทัศน์" : "Vision"}</h3>
            </div>
          ...
          <div className="bg-navy/40 rounded-xl border border-white/5 p-6">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-thai font-bold text-white">{lang === "th" ? "พันธกิจ" : "Mission"}</h3>
            </div>
            <ul className="text-white/50 font-body text-sm leading-relaxed space-y-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          {[
            ...
            <div key={s.label} className="bg-navy/40 rounded-xl border border-white/5 p-4 text-center">
              <p className="font-display text-2xl text-amber font-bold">{s.value}</p>
              <p className="text-white/40 text-xs font-thai mt-1">{s.label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mb-10">
          <FilmStrip color="purple" size="md">
            <h2 className="font-thai font-bold text-xl text-white">{lang === "th" ? "ประวัติความเป็นมา" : "History"}</h2>
          </FilmStrip>
          <div className="mt-6 space-y-6 relative before:absolute before:left-4 before:top-0 before:bottom-0 before:w-px before:bg-white/10">
            {timeline.map((t) => (
              <div key={t.year} className="flex gap-4 pl-4">
                ...
                <div>
                  ...
                  <p className="text-white font-thai font-semibold text-sm">{t.title}</p>
                  <p className="text-white/40 font-body text-xs mt-1">{t.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          <FilmStrip color="orange" size="md">
            <h2 className="font-thai font-bold text-xl text-white flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
            {teamMembers.map((m) => (
              <div key={m.name} className="bg-navy/40 rounded-xl border border-white/5 p-4 flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-purple/30 to-navy flex items-center justify-center flex-shrink-0">
                  <span className="text-white/30 font-display font-bold">{m.name.charAt(0)}</span>
                </div>
                <div>
                  <p className="text-white font-thai text-sm font-semibold">{m.name}</p>
                  <p className="text-white/40 font-thai text-xs">{m.role}</p>
                </div>
              </div>
            ))}
          </div>
