              ...
              <StaggerChildren staggerDelay={0.03} className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-3">
                {paginated.map((film) => (
                  <StaggerItem key={film.id}>
                    <a href={`/films/${film.slug}`} className="group relative aspect-[2/3] rounded-lg overflow-hidden bg-[var(--ct-bg-elevated)] border border-[var(--ct-border)] hover:-translate-y-1 hover:border-[#EC1C72]/30 hover:shadow-[0_10px_40px_rgba(236,28,114,0.1)] transition-all duration-300">
                      {film.posterUrl ? (
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-[var(--ct-bg-page)] to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <p className="text-[#F6A51B] text-[9px]">{film.year}</p>
                      </div>
                    </a>
                  </StaggerItem>
                ))}
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-2 mt-12">
            <button
              ...
              disabled={page === 1}
              className="px-4 py-2 bg-[var(--ct-bg-elevated)] border border-[var(--ct-border)] text-[var(--ct-text-muted)] text-sm rounded-xl disabled:opacity-30 hover:border-[#EC1C72]/30 hover:text-white transition-all font-thai"
            >
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <button
                ...
                className={`w-10 h-10 text-sm rounded-xl transition-all font-thai ${
                  p === page
                    ? "bg-[#EC1C72] text-[var(--ct-text-primary)] shadow-[0_0_20px_rgba(236,28,114,0.3)]"
                    : "bg-[var(--ct-bg-elevated)] border border-[var(--ct-border)] text-[var(--ct-text-muted)] hover:border-[#EC1C72]/30 hover:text-white"
                }`}
              >
                {p}
              </button>
...
function FilmListItem({ film, lang, index }: { film: (typeof films)[0]; lang: string; index: number }) {
  ...
  return (
    ...
    >
      <div className="w-16 md:w-20 h-24 md:h-[120px] flex-shrink-0 rounded-lg bg-gradient-to-br from-[#702874]/20 to-[var(--ct-bg-page)] overflow-hidden relative group-hover:shadow-[0_0_20px_rgba(236,28,114,0.1)] transition-shadow">
        {film.posterUrl ? (
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-center">
        ...
        <div className="flex flex-wrap items-center gap-2 mt-2">
          <span className="px-2 py-0.5 bg-[#F6A51B]/15 text-[#F6A51B] text-xs rounded-md font-bold font-mono">{film.year}</span>
          {film.duration && <span className="text-[var(--ct-text-faint)] text-xs font-thai">{film.duration} {lang === "th" ? "นาที" : "min"}</span>}
