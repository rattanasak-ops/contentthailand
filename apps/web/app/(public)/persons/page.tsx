...
function PersonCardGrid({ p, lang }: { p: (typeof persons)[0]; lang: string }) {
  ...
  return (
    ...
    >
      {/* Avatar */}
      <div className="relative aspect-square rounded-xl bg-gradient-to-br from-[#F6A51B]/10 to-[var(--ct-bg-elevated)] border border-[var(--ct-border)] group-hover:border-[#F6A51B]/40 transition-all duration-300 overflow-hidden mb-3 group-hover:shadow-[0_15px_40px_rgba(246,165,27,0.15)]">
        {/* Mouse glow */}
        <motion.div
...
function PersonCardPoster({ p, lang }: { p: (typeof persons)[0]; lang: string }) {
  ...
  return (
    <Link
      href={`/persons/${p.slug}`}
      className="group relative aspect-[2/3] rounded-lg overflow-hidden bg-[var(--ct-bg-elevated)] border border-[var(--ct-border)] hover:-translate-y-1 hover:border-[#F6A51B]/40 hover:shadow-[0_10px_40px_rgba(246,165,27,0.15)] transition-all duration-300"
    >
      {p.photoUrl ? (
...
function PersonListItem({ p, lang, index }: { p: (typeof persons)[0]; lang: string; index: number }) {
  ...
  return (
    ...
    >
      <div className="w-14 h-14 md:w-16 md:h-16 flex-shrink-0 rounded-full bg-gradient-to-br from-[#F6A51B]/10 to-[var(--ct-bg-elevated)] overflow-hidden relative group-hover:shadow-[0_0_20px_rgba(246,165,27,0.15)] transition-shadow border border-[var(--ct-border)] group-hover:border-[#F6A51B]/40">
        {p.photoUrl ? (
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-center">
        ...
        <p className="text-[var(--ct-text-muted)] text-sm font-body">{altName}</p>
