      ...
      <div className="aspect-[2/3] bg-gradient-to-br from-purple/30 to-midnight relative overflow-hidden">
        ...
        <div className="absolute bottom-2 left-2 flex gap-1">
          {film.genres.slice(0, 2).map((genre) => (
            <span
              key={genre.slug}
              className="px-1.5 py-0.5 bg-midnight/80 text-white/70 text-[10px] rounded font-thai"
            >
        ...
        {/* Hover overlay */}
        <div className="absolute inset-0 bg-gradient-to-t from-midnight via-midnight/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-3">
          {synopsis && (
